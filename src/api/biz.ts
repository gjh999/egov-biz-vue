...
// ================================================================ 기업 업무 전용
// 아래는 포털에 없는 기업 업무 템플릿 고유 기능이다.

/** 공통코드 — 업무 화면의 셀렉트 값이 여기서 나온다 (관리자 전용) */
export const codeApi = {
  /** 분류코드 목록 (코드 묶음의 큰 갈래) */
  classList: (params: SearchParams = {}) =>
    api.get<PagedResult<CommonClassCode>>('/codes/classes', { pageIndex: params.pageIndex ?? 1 }),

  createClass: (values: FormValues) => api.post<unknown>('/codes/classes', values),

  updateClass: (clCode: string, values: FormValues) =>
    api.put<unknown>(`/codes/classes/${encodeURIComponent(clCode)}`, values),

  removeClass: (clCode: string) => api.delete<unknown>(`/codes/classes/${encodeURIComponent(clCode)}`),

  /** 코드 목록 (예: COM030 일정구분) */
  list: (params: SearchParams = {}) =>
    api.get<PagedResult<CommonCode>>('/codes', {
      pageIndex: params.pageIndex ?? 1,
      searchCondition: params.searchCondition,
      searchKeyword: params.searchKeyword,
    }),

  detail: (codeId: string) => api.get<CommonCode>(`/codes/${encodeURIComponent(codeId)}`),

  create: (values: FormValues) => api.post<unknown>('/codes', values),

  update: (codeId: string, values: FormValues) =>
    api.put<unknown>(`/codes/${encodeURIComponent(codeId)}`, values),

  remove: (codeId: string) => api.delete<unknown>(`/codes/${encodeURIComponent(codeId)}`),

  /** 상세코드 목록 (예: 1=회의, 2=세미나) */
  details: (codeId: string, params: SearchParams = {}) =>
    api.get<PagedResult<CommonDetailCode>>(`/codes/${encodeURIComponent(codeId)}/details`, {
      pageIndex: params.pageIndex ?? 1,
    }),

  createDetail: (codeId: string, values: FormValues) =>
    api.post<unknown>(`/codes/${encodeURIComponent(codeId)}/details`, values),

  updateDetail: (codeId: string, code: string, values: FormValues) =>
    api.put<unknown>(`/codes/${encodeURIComponent(codeId)}/details/${encodeURIComponent(code)}`, values),

  removeDetail: (codeId: string, code: string) =>
    api.delete<unknown>(`/codes/${encodeURIComponent(codeId)}/details/${encodeURIComponent(code)}`),
}

/** 업무사용자(직원) 관리 — 일반회원과 저장 테이블이 다르다 (관리자 전용) */
export const userApi = {
  list: (params: SearchParams = {}) =>
    api.get<PagedResult<MemberListItem>>('/admin/users', {
      pageIndex: params.pageIndex ?? 1,
      searchCondition: params.searchCondition,
      searchKeyword: params.searchKeyword,
    }),

  detail: (emplyrId: string) => api.get<MemberListItem>(`/admin/users/${encodeURIComponent(emplyrId)}`),

  create: (values: FormValues) => api.post<unknown>('/admin/users', values),

  update: (emplyrId: string, values: FormValues) =>
    api.put<unknown>(`/admin/users/${encodeURIComponent(emplyrId)}`, values),

  /** 서비스가 콤마로 구분된 복수 ID 를 받으므로 일괄 삭제도 같은 API 로 처리한다 */
  remove: (emplyrIds: string) => api.delete<unknown>(`/admin/users/${encodeURIComponent(emplyrIds)}`),
}

/** 시스템 운영 관리 — 메뉴 · 프로그램 · 부재 · 접속정책 (관리자 전용) */
export const adminApi = {
  menuList: (params: SearchParams = {}) =>
    api.get<PagedResult<MenuItem>>('/admin/menus', { pageIndex: params.pageIndex ?? 1 }),

  createMenu: (values: FormValues) => api.post<unknown>('/admin/menus', values),

  updateMenu: (menuNo: number | string, values: FormValues) =>
    api.put<unknown>(`/admin/menus/${menuNo}`, values),

  removeMenu: (menuNos: string) => api.delete<unknown>(`/admin/menus/${encodeURIComponent(menuNos)}`),

  programList: (params: SearchParams = {}) =>
    api.get<PagedResult<ProgramItem>>('/admin/programs', { pageIndex: params.pageIndex ?? 1 }),

  createProgram: (values: FormValues) => api.post<unknown>('/admin/programs', values),

  updateProgram: (progrmFileNm: string, values: FormValues) =>
    api.put<unknown>(`/admin/programs/${encodeURIComponent(progrmFileNm)}`, values),

  removeProgram: (progrmFileNms: string) =>
    api.delete<unknown>(`/admin/programs/${encodeURIComponent(progrmFileNms)}`),

  absenceList: (params: SearchParams = {}) =>
    api.get<PagedResult<UserAbsence>>('/admin/absences', { pageIndex: params.pageIndex ?? 1 }),

  createAbsence: (values: FormValues) => api.post<unknown>('/admin/absences', values),

  updateAbsence: (userId: string, values: FormValues) =>
    api.put<unknown>(`/admin/absences/${encodeURIComponent(userId)}`, values),

  removeAbsence: (userId: string) => api.delete<unknown>(`/admin/absences/${encodeURIComponent(userId)}`),

  loginPolicyList: (params: SearchParams = {}) =>
    api.get<PagedResult<LoginPolicy>>('/admin/login-policies', { pageIndex: params.pageIndex ?? 1 }),

  createLoginPolicy: (values: FormValues) => api.post<unknown>('/admin/login-policies', values),

  updateLoginPolicy: (emplyrId: string, values: FormValues) =>
    api.put<unknown>(`/admin/login-policies/${encodeURIComponent(emplyrId)}`, values),

  removeLoginPolicy: (emplyrId: string) =>
    api.delete<unknown>(`/admin/login-policies/${encodeURIComponent(emplyrId)}`),
}

/**
 * 로그 조회 · 접속 통계 (관리자 전용).
 *
 * 로그는 조회만 제공한다 — 기록은 시스템이 자동으로 남기고, 감사 추적 대상이라
 * 화면에서 수정·삭제할 수 있으면 안 되기 때문이다.
 */
export const logApi = {
  loginLogList: (params: SearchParams = {}) =>
    api.get<PagedResult<LoginLog>>('/admin/logs/login', { pageIndex: params.pageIndex ?? 1 }),

  loginLogDetail: (logId: string) => api.get<LoginLog>(`/admin/logs/login/${encodeURIComponent(logId)}`),

  sysLogList: (params: SearchParams = {}) =>
    api.get<PagedResult<SysLog>>('/admin/logs/system', { pageIndex: params.pageIndex ?? 1 }),

  sysLogDetail: (requstId: string) => api.get<SysLog>(`/admin/logs/system/${encodeURIComponent(requstId)}`),

  /** 접속 통계 — 집계 기준(statsKind)에 따라 결과의 의미가 달라진다 */
  connectStats: (params: Record<string, string> = {}) =>
    api.get<{ resultList: Record<string, unknown>[] }>('/admin/stats/connect', params),
}
