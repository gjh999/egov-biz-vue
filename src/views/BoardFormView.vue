 ...
 * 게시물 등록 / 수정 폼.
 *
 * 두 화면의 입력 항목이 같아 하나로 합쳤다. 다른 점은 (1) 초기값을 불러오는지,
 * (2) 어떤 API 를 호출하는지 뿐이다. 모드는 라우트 props 로 받는다.
 */
...
onMounted(async () => {
  try {
    const master = await boardMasterApi.detail(bbsId.value)
    maxFiles.value = master.fileAtchPosblAt === 'Y' ? master.posblAtchFileNumber : 0
  } catch {
    maxFiles.value = 0
  }
  ...
  try {
    ...
    if (props.mode === 'edit') {
      subject.value = detail.boardVO.nttSj
      content.value = detail.boardVO.nttCn
    } else {
      // 답변은 제목에 원글 제목을 이어 붙이는 관례를 따른다
      subject.value = `RE: ${detail.boardVO.nttSj}`
    }
  } catch (e) {
...
async function handleSubmit() {
  ...
  try {
    const input = { nttSj: subject.value, nttCn: content.value, files: files.value }
    if (props.mode === 'create') {
      await boardApi.create(bbsId.value, input)
    } else if (props.mode === 'edit') {
      await boardApi.update(bbsId.value, nttId.value, input)
    } else {
      await boardApi.reply(bbsId.value, nttId.value, input)
    }
